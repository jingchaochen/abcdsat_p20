/***************************************************************************************[Solver_Mix.cc]
Maple_mix -- 2020 by Jingchao Chen
...
// simplify All
//
CRef Solver_Mix::simplePropagate()
{
...
void Solver_Mix::simpleUncheckEnqueue(Lit p, CRef from){
...
void Solver_Mix::cancelUntilTrailRecord()
{
...
void Solver_Mix::litsEnqueue(int cutP, Clause& c)
{
...
bool Solver_Mix::removed(CRef cr) {
    return ca[cr].mark() == 1;
}

void Solver_Mix::simpleAnalyze(CRef confl, vec<Lit>& out_learnt, vec<CRef>& reason_clause, bool True_confl)
{
    int pathC = 0;
    Lit p = lit_Undef;
...
void Solver_Mix::simplifyLearnt(Clause& c)
{
...
bool Solver_Mix::simplifyLearnt_core()
{
    int ci, cj, li, lj;
    bool sat, false_lit;
...
bool Solver_Mix::simplifyLearnt_tier2()
{
    int ci, cj, li, lj;
    bool sat, false_lit;
    unsigned int nblevels;
...
//
Var Solver_Mix::newVar(bool sign, bool dvar)
{
    int v = nVars();
...
bool Solver_Mix::addClause_(vec<Lit>& ps)
{
...
void Solver_Mix::attachClause(CRef cr) {
...
void Solver_Mix::detachClause(CRef cr, bool strict) {
    const Clause& c = ca[cr];
    assert(c.size() > 1);
    OccLists<Lit, vec<Watcher>, WatcherDeleted>& ws = c.size() == 2 ? watches_bin : watches;
...
void Solver_Mix::removeClause(CRef cr) {
    Clause& c = ca[cr];
...
bool Solver_Mix::satisfied(const Clause& c) const {
    for (int i = 0; i < c.size(); i++)
        if (value(c[i]) == l_True)
            return true;
    return false; }
...
//
void Solver_Mix::cancelUntil(int bLevel) {
...
inline Solver_Mix::ConflictData Solver_Mix::FindConflictLevel(CRef cind)
{
	ConflictData data;
...
void Solver_Mix::analyze(CRef confl, vec<Lit>& out_learnt, int& out_btlevel, int& out_lbd)
{
    int pathC = 0;
    Lit p     = lit_Undef;
...
bool Solver_Mix::binResMinimize(vec<Lit>& out_learnt)
{
...
bool Solver_Mix::litRedundant(Lit p, uint32_t abstract_levels)
{
...
|________________________________________________________________________________________________@*/
void Solver_Mix::analyzeFinal(Lit p, vec<Lit>& out_conflict)
{
    out_conflict.clear();
    out_conflict.push(p);
...
void Solver_Mix::uncheckedEnqueue(Lit p, int level, CRef from)
{
    assert(value(p) == l_Undef);
    Var x = var(p);
    if (!VSIDS){
...
void Solver_Mix::reduceDB_Tier2()
{
    int i, j;
...
void Solver_Mix::removeSatisfied(vec<CRef>& cs)
{
    int i, j;
...
void Solver_Mix::safeRemoveSatisfied(vec<CRef>& cs, unsigned valid_mark)
{
    int i, j;
    for (i = j = 0; i < cs.size(); i++){
        Clause& c = ca[cs[i]];
...
void Solver_Mix::rebuildOrderHeap()
{
    vec<Var> vs;
...
|________________________________________________________________________________________________@*/
lbool Solver_Mix::search(int& nof_conflicts)
{
    assert(ok);
    int         backtrack_level;
    int         lbd;
    ...
    if(nof_conflicts>1000000) nof_conflicts=1000000;
    ...
    for (;;){
        ...
        if (confl != CRef_Undef){
           ...
           // if(nof_conflicts<-10000){
            ...
            if ((confl_to_chrono < 0 || confl_to_chrono <= (int)conflicts) && chrono > -1 && (decisionLevel() - backtrack_level) >= chrono)
            {
				++chrono_backtrack;
				cancelUntil(data.nHighestLevel -1);
			}
...
double Solver_Mix::progressEstimate() const
{
    double  progress = 0;
    double  F = 1.0 / nVars();
...
//           double endtime=cpuTime();
        ...
        if (switch_mode){ 
            ...
            if (VSIDS) printf("c Mix Switched to VSIDS.\n");
...
void Solver_Mix::toDimacs(FILE* f, Clause& c, vec<Var>& map, Var& max)
{
...
void Solver_Mix::toDimacs(const char *file, const vec<Lit>& assumps)
{
...
void Solver_Mix::toDimacs(FILE* f, const vec<Lit>& assumps)
{
...
void Solver_Mix::relocAll(ClauseAllocator& to)
{
    // All watchers:
    //
...
void Solver_Mix::garbageCollect()
{
...
bool Solver_Mix::hasBin(Lit p, Lit q)
{
    vec<Watcher>&  wbin  = watches_bin[~p];
    for(int k = 0;k<wbin.size();k++) {
	  Lit imp = wbin[k].blocker;
...
inline bool Solver_Mix :: non_decision(Lit p)
{    Var v=var(p);
     if (value(v) != l_Undef || !decision[v]) return true;
     return false;
}

void Solver_Mix:: putbin(Lit p, Lit q)
{
